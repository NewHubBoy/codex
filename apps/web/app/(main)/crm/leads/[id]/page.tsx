  ...
  const tabItems = [
    {
      ...
      children: (
        <Descriptions column={2} bordered>
          ...
          <Descriptions.Item label="负责人邮箱">{lead.owner?.email || '-'}</Descriptions.Item>
        </Descriptions>
      ),
    },
    {
    ...
    },
    {
      ...
      children: (
        <Descriptions column={2} bordered>
          <Descriptions.Item label="创建时间">{new Date(lead.createdAt).toLocaleString('zh-CN')}</Descriptions.Item>
          <Descriptions.Item label="最后更新时间">{new Date(lead.updatedAt).toLocaleString('zh-CN')}</Descriptions.Item>
        ...
        extra={[
          ...
          ...(isEditing
            ? [
                ...
                <Button key="save" type="primary" loading={submitLead.isPending || updateLead.isPending} onClick={handleSave}>
                  保存
                </Button>,
              ]
            : [
                ...
                <Button key="convert" icon={<SwapOutlined />} type="primary" disabled={lead.status === 'CONVERTED'}>
                  转换为客户
                </Button>,
              ]),
          <Popconfirm key="delete" title="确认删除" description="确定要删除这条线索吗？" onConfirm={handleDelete} okText="确认" cancelText="取消">
          ...
          {isEditing ? (
            <Form form={form} layout="vertical" requiredMark="optional">
              ...
              <Space style={{ width: '100%' }} size={16}>
                <Form.Item name="email" label="邮箱" style={{ flex: 1 }} rules={[{ type: 'email', message: '请输入有效的邮箱地址' }]}>
              ...
              <Form.Item name="expectedValue" label="预期金额">
                <InputNumber style={{ width: '100%' }} placeholder="请输入预期金额" min={0} precision={2} prefix="¥" />
              </Form.Item>
            ...
            <Descriptions column={2} bordered>
              ...
              <Descriptions.Item label="优先级">
                <Tag color={ratingColors[lead.rating || ''] || 'default'}>{lead.rating || '-'}</Tag>
              </Descriptions.Item>
              <Descriptions.Item label="来源">{lead.source ? sourceLabels[lead.source] || lead.source : '-'}</Descriptions.Item>
              <Descriptions.Item label="预期金额">{lead.expectedValue ? `¥${lead.expectedValue.toLocaleString()}` : '-'}</Descriptions.Item>
